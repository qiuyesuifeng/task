...
// refer to github.com/astaxie/beego/toolbox/task.go
package task
...
var (
	...
	AdminTasks        map[string]Tasker
	AdminTaskCommands map[string]TaskerCommand
...
func NewTask(taskName string, spec string, f TaskFunc) *Task {
	task := &Task{
		TaskName: taskName,
		DoFunc:   f,
		ErrLimit: 0,
...
// set arg
func (tk *Task) SetArg(arg interface{}) {
	tk.Arg = arg
}

// run task
func (tk *Task) Run() error {
	err := tk.DoFunc(tk.Arg)
	if err != nil {
...
// run task silent
func (tk *Task) RunSilent() {
	tk.TaskLock.Lock()
	defer tk.TaskLock.Unlock()
	_ = tk.DoFunc(tk.Arg)
	tk.Arg = nil
}
...
func (t *Task) parse(spec string) *Schedule {
	...
	if len(fields) != 5 && len(fields) != 6 {
		logging.Fatalf("Expected 5 or 6 fields, found %d: %s", len(fields), spec)
	}
...
func (t *Task) parseSpec(spec string) *Schedule {
	...
	logging.Errorf("Unrecognized descriptor: %s", spec)
	return nil
}
...
func StartTask() {
	...
	logging.Infof("Task stop!")
	ServiceChan <- false
}

// process task commands
func processTaskCommand() {
	now := time.Now().Local()
	for k, v := range AdminTaskCommands {
		if v.opType == "add" {
			v.tasker.SetNext(now)
			AdminTasks[k] = v.tasker
		} else if v.opType == "remove" {
			delete(AdminTasks, k)
		} else {
			logging.Warningf("opType(%s) is Bad", v.opType)
		}
	}

	// clear items in AdminTaskCommands
	AdminTaskCommands = make(map[string]TaskerCommand)
}
...
func run() {
	...
	AdminWaitGroup.Add(1)
	StartService()

	for {
		// check if there is any new task
		AdminLock.Lock()
		if len(AdminTaskCommands) > 0 {
			processTaskCommand()
		}
		AdminLock.Unlock()

		sortList := NewMapSorter(AdminTasks)
		sortList.Sort()
		var effective time.Time
		if len(AdminTasks) == 0 || sortList.Vals[0].GetNext().IsZero() {
...
// default task
func DefaultTask(arg interface{}) error {
	fmt.Println(time.Now().String() + " Default Task Test")
	return nil
}
...
// add task with name to AdminTasks
func AddTask(taskName string, t Tasker) {
	AdminTasks[taskName] = t
}

// add new task with name to AdminTaskCommands, then can be merge into AdminTasks
func AddTaskCommand(taskName string, t Tasker, opType string) {
	AdminLock.Lock()
	task := TaskerCommand{t, opType}
	AdminTaskCommands[taskName] = task
	AdminLock.Unlock()

	CommandChan <- true
	logging.Infof("Add task command - [%s:%s]", taskName, opType)
}
...
func getRange(expr string, r bounds) uint64 {
	...
	if lowAndHigh[0] == "*" || lowAndHigh[0] == "?" {
		...
		default:
			logging.Errorf("Too many hyphens: %s", expr)
		}
	}
	...
	default:
		logging.Errorf("Too many slashes: %s", expr)
	}

	if start < r.min {
		logging.Errorf("Beginning of range (%d) below minimum (%d): %s", start, r.min, expr)
	}
	if end > r.max {
		logging.Errorf("End of range (%d) above maximum (%d): %s", end, r.max, expr)
	}
	if start > end {
		logging.Errorf("Beginning of range (%d) beyond end of range (%d): %s", start, end, expr)
	}
...
func mustParseInt(expr string) uint {
	...
	if err != nil {
		logging.Errorf("Failed to parse int from %s: %s", expr, err)
	}
	if num < 0 {
		logging.Errorf("Negative number (%d) not allowed: %s", num, expr)
	}
...
func init() {
	...
	AdminTasks = make(map[string]Tasker)
	AdminTaskCommands = make(map[string]TaskerCommand)
